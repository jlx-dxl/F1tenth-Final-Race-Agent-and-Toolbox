...
########################################### Callback ############################################
    ...
    def scan_callback(self, scan_msg: LaserScan):
        ...
        cared_ranges = np.where(((cared_ranges >= scan_msg.range_min) & (cared_ranges <= scan_msg.range_max)), cared_ranges, 30.0)
        ...
        self.range_rate = range_diff / (self.get_clock().now().nanoseconds / 1e9 - self.last_time)
    ...
    def pose_callback(self, pose_msg):
        ...
        if self.filtered_range_rate < 0 and self.mean_cared_ranges < distance_threshold and abs(self.filtered_range_rate) < speed_tolerance:
              #target_v = self.filtered_range_rate + self.speed
              target_v = 0.0
        ...
        #self.get_logger().info('speed: %8f, steer: %8f' % (target_v, self.get_steer(error)))
        self.get_logger().info("Range Rate:%12f; Current Speed:%12f; Opp Speed:%12f; Traget:12f" % (self.filtered_range_rate, self.speed, self.speed + self.filtered_range_rate, target_v))
